...
export class TrackStore {
    ...
    loadSongs(playlist_id: string, tracks: TrackList) {
        this.db.prepare("INSERT OR REPLACE INTO playlists (playlist_id, last_updated) VALUES (?, ?);").run(playlist_id, new Date().getTime());
        const songStmt = this.db.prepare("INSERT OR REPLACE INTO songs (song_id, preview_url, img_url, title, artists, add_time) VALUES (?,?,?,?,?,?);"); 
        ...
        const txn = this.db.transaction((tracks: TrackList) => {
            tracks.forEach((track) => {
                try {
                    songStmt.run(track.id, track.preview_url, null, track.title, track.artists.map((artist) => 
                        Buffer.from(artist).toString('base64')).join(','), 
                        new Date().getTime());
...
function open_db() {
    ...
    + ");").run();
    db.prepare("CREATE TABLE IF NOT EXISTS contents (\n"
